...
Multicam-Studio-1.3.0-Sources.zip. Redistribute that archive alongside the binaries.
...
def sources_zip(destination,work):
    with zipfile.ZipFile(destination,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as archive:
        ...
        for pattern in ['desktop.py','server.py','multicam_edit.py','effects.py','highlights.py','audio_batch.py','requirements.txt','README.md','CHANGELOG.md','docs/*.md','tests/*.py','tests/*.cjs','Start Multicam Studio.command','packaging/*.py','packaging/*.sh','packaging/*.spec','packaging/*.txt','packaging/*.plist','packaging/*.md','packaging/*.icns','web/*']:
